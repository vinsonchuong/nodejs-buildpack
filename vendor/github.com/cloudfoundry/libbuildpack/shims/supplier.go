...
func (s *Supplier) EnsureNoV2AfterV3() error {
	allBuildpacks, err := filepath.Glob(filepath.Join(string(filepath.Separator), "tmp", "buildpacks", "*"))
	if err != nil {
		return err
	}
	fmt.Println("ALL BUILDPACKS")
	fmt.Println(allBuildpacks)

	v3Buildpacks, err := filepath.Glob(filepath.Join(string(filepath.Separator), "tmp", "buildpacks", "*", "order.toml"))
	if err != nil {
		return err
	}
	fmt.Println("V3 BUILDPACKS")
	...
	fmt.Println("buildpacksDownloaded")
	fmt.Println(buildpacksDownloaded)
	...
	fmt.Println("V3 BUILDPACKS DOWNLOADED")
	fmt.Println(v3BuildpacksDownloaded)
