...
var _ = Describe("CF NodeJS Buildpack", func() {
	const serviceOffering string = "newrelic"

	var (
		app, serviceBrokerApp         *cutlass.App
		serviceBrokerURL, serviceName string
	)
	...
	AfterEach(func() {
		...
		RunCF("cf", "purge-service-offering", "-f", serviceOffering)
		RunCF("cf", "delete-service-broker", "-f", serviceOffering)
